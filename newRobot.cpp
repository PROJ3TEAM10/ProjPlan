...
void doCompletion(){
	...
	int left = (cameraView.width)/8;													//if the line turns left, it goes throught this pixel coloumn
	int right = 7*(cameraView.width)/8;													//if the line turns right, it goes throught this pixel coloumn
	while(1){
		takePicture();
		...
		for(int i=0; i<cameraView.height/4; i++){
			//left turn check
			int whitePixLeft = get_pixel(cameraView, i, left, 3);
			if(doWhiteCheck(whitePixLeft)==1){
				doLeftTurn(3);
				}
			//if it isn't turning left do a right turn check
			if(leftt==false){
				int whitePixRight = get_pixel(cameraView, i, right, 3);
